...
==================
쿠버네티스(Kubernetes) 서비스 브로커는 쿠버네티스 서비스를 파스-타(PaaS-TA) 서비스로 제공합니다.
이 서비스 브로커는 클라우드 컨트롤러와 서비스 브로커 간의 v2 서비스 API를 보여줍니다.
이 API는 클라우드 컨트롤러 API와 혼동되어서는 안됩니다.

이 브로커에는 쿠버네티스 서버가 포함되어 있지 않습니다.
대신 쿠버네티스 서버의 서비스를 관리하는 쿠버네티스 자바 브로커를 배포하는 것을 의미합니다.

쿠버네티스 서비스 브로커는 쿠버네티스 서버와 OpenPaas(클라우드 파운드리)간의 서비스를 제공하는 것을 말하며, 독립 실행하여 실행되는 쿠버네티스 서버 응용프로그램은 별도로 지원하지 않습니다.

이 브로커가 수행하는 쿠버네티스 관리 작업은 다음과 같습니다.
- 쿠버네티스 서비스의 카탈로그 등록
- 쿠버네티스 인스턴스 프로비저닝 생성 (네임스페이스 및 유저 생성)
- 쿠버네티스 인스턴스 프로비저닝 플랜의 갱신
- 쿠버네티스 인스턴스 프로비저닝 해제 (유저 및 네임스페이스 삭제)
이 브로커에서는 쿠버네티스 서비스에 대한 바인딩/언바인딩은 제공하지 않습니다.
